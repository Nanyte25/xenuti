  ...
  context 'fetch_source' do
    it 'should check out repo to the latest version' do
      Dir.mktmpdir do |tmpdir|
        ...
        expect(Dir.compare(config[:content_update][:repo], tmpdir)).to be_true
        expect(config[:content_update][:source]).to be_eql(tmpdir)
      end
    end
    ...
    it 'should update repo to the latest version' do
      ...
      expect(Dir.compare(config[:content_update][:repo], OUTDATED_REPO)).to \
        be_true
    end
  end
