...
describe Xenuti::ScriptReport do
  ...
  describe '::diff' do
    it 'should diff correctly' do
      ...
      expect(new_report['new_messages']).to eq([new_warn, ignored_new])
      expect(new_report['fixed_messages']).to eq([old_warn, ignored_old])
      expect([:a, :b].include? new_report['messages'][0][:name]).to be true
      expect([:a, :b].include? new_report['messages'][1][:name]).to be true
    end
    ...
    it 'should ignore fields when specified as array' do
      ...
      expect(new_report['new_messages'].size).to eq(1)
      expect(new_report['new_messages'].first[:name]).to eq(:b)

      # fixed_messages containt old_warn
      expect(new_report['fixed_messages'].size).to eq(1)
      expect(new_report['fixed_messages'].first[:name]).to eq(:c)
    ...
    it 'should not modify old report passed as argument' do
      new_report.diff!(old_report)
      expect(old_report['new_messages']).to be nil
      expect(old_report['fixed_messages']).to be nil
      new_report['messages'] << 'foo'
      expect(old_report['messages'].size).to eq(3)
    end
  end
end
